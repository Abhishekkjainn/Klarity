...
// // =================================================================
// // 4. POLLING ENGINE & ALERTING LOGIC
// // =================================================================
...
//   try {
//     let token; if (req.cookies.jwt) { token = req.cookies.jwt; } if (!token) { return next(new AppError('You are not logged in.', 401)); }
//     const decoded = jwt.verify(token, process.env.JWT_SECRET);
//     const { rows } = await db.query('SELECT id, email, created_at FROM users WHERE id = $1', [decoded.user.id]); const currentUser = rows[0];
