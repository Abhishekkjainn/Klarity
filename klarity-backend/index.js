...
// const ensureSchema = async () => { /* ... (code unchanged) ... */ 
...
//     const createLocksTableQuery = `CREATE TABLE IF NOT EXISTS locks (lock_name VARCHAR(255) PRIMARY KEY, locked_at TIMESTAMPTZ);`;

//     try { await db.query(createUsersTableQuery); 
...
//         await db.query(createLocksTableQuery); 
//         console.log('✅ "locks" table is ready.');
...
//     try {
//         // --- UPGRADED: Database-backed lock to prevent overlaps ---
//         // Try to acquire a lock. If it's older than 2 minutes, overwrite it.
//         await client.query('BEGIN');
//         const lock = await client.query(`
//             INSERT INTO locks (lock_name, locked_at) 
//             VALUES ('runHealthChecks', NOW()) 
//             ON CONFLICT (lock_name) DO UPDATE 
//             SET locked_at = NOW()
//             WHERE locks.locked_at < NOW() - INTERVAL '2 minutes';
//         `);
        
//         if (lock.rowCount === 0) {
//             console.log('Health check is already in progress (lock held). Skipping this run.');
//             await client.query('ROLLBACK');
//             return;
//         }

//         console.log(`[${new Date().toISOString()}] Lock acquired. Running health checks...`);
//         const query = `SELECT m.*, u.email as owner_email FROM monitors m JOIN users u ON m.user_id = u.id WHERE m.is_active = true AND (m.last_checked_at IS NULL OR m.last_checked_at <= NOW() - (m.check_interval_seconds * '1 second'::interval));`;
//         const { rows: dueMonitors } = await client.query(query);
//         if (dueMonitors.length === 0) { console.log('No monitors due for a check.'); }
//         else { console.log(`Found ${dueMonitors.length} monitor(s) to check.`); }
...
// // --- 🚀 FINAL, RICH ANALYTICS ROUTES ---
// const analyticsRouter = express.Router();
// analyticsRouter.use(protect);

// // 1. Monitor Overview (Replaces Summary) - HIGHLY ACCURATE
// analyticsRouter.get('/:monitorId/overview', async (req, res, next) => {
//     try {
//         const { monitorId } = req.params;
//         const { period = '24h' } = req.query;
//         const intervalMap = { '24h': '1 day', '7d': '7 days', '30d': '30 days' };
//         if (!intervalMap[period]) { return next(new AppError('Invalid period. Use 24h, 7d, or 30d.', 400)); }

//         const statsQuery = `
//             SELECT 
//                 COUNT(*) AS "totalChecks",
//                 TRUNC(AVG(response_time_ms)) AS "averageLatency",
//                 TRUNC((COUNT(*) FILTER (WHERE was_successful) * 100.0 / COUNT(*)), 2) AS "uptimePercentage",
//                 COUNT(*) FILTER (WHERE NOT was_successful) AS "totalFailures",
//                 TRUNC(percentile_cont(0.95) WITHIN GROUP (ORDER BY response_time_ms)) AS "p95Latency",
//                 TRUNC(percentile_cont(0.99) WITHIN GROUP (ORDER BY response_time_ms)) AS "p99Latency"
//             FROM health_checks WHERE monitor_id = $1 AND timestamp >= NOW() - $2::interval;
//         `;
//         const statusQuery = `
//             SELECT status_code, COUNT(*) as count FROM health_checks
//             WHERE monitor_id = $1 AND timestamp >= NOW() - $2::interval AND status_code IS NOT NULL
//             GROUP BY status_code ORDER BY count DESC;
//         `;
//         const [statsResult, statusResult] = await Promise.all([
//             db.query(statsQuery, [monitorId, intervalMap[period]]),
//             db.query(statusQuery, [monitorId, intervalMap[period]])
//         ]);

//         res.status(200).json({ status: 'success', statusCode: 200, message: `Overview for the last ${period} retrieved.`, data: { ...statsResult.rows[0], statusCounts: statusResult.rows } });
//     } catch (error) { next(error); }
// });

// // 2. Latency Timeseries
// analyticsRouter.get('/:monitorId/latency-series', async (req, res, next) => {
//     try {
//         const { monitorId } = req.params;
//         const { period = '24h' } = req.query;
//         const intervalMap = { '24h': '1 day', '7d': '7 days', '30d': '30 days' };
//         const granularityMap = { '24h': 'hour', '7d': 'day', '30d': 'day' };
//         if (!intervalMap[period]) { return next(new AppError('Invalid period.', 400)); }
//         const query = `SELECT date_trunc($2, timestamp) as time_bucket, ROUND(AVG(response_time_ms)) as avg_latency FROM health_checks WHERE monitor_id = $1 AND timestamp >= NOW() - $3::interval AND was_successful = true GROUP BY time_bucket ORDER BY time_bucket ASC;`;
//         const { rows } = await db.query(query, [monitorId, granularityMap[period], intervalMap[period]]);
//         res.status(200).json({ status: 'success', statusCode: 200, message: `Latency series for the last ${period} retrieved.`, data: rows });
//     } catch (error) { next(error); }
// });

// // 3. Incidents Log - HIGHLY ACCURATE
// analyticsRouter.get('/:monitorId/incidents', async (req, res, next) => {
//     try {
//         const { monitorId } = req.params;
...
//         if (!intervalMap[period]) { return next(new AppError('Invalid period.', 400)); }
//         const query = `
//             WITH grouped_checks AS (
//                 SELECT
//                     timestamp, was_successful,
//                     ROW_NUMBER() OVER (ORDER BY timestamp) - ROW_NUMBER() OVER (PARTITION BY was_successful ORDER BY timestamp) as incident_group
//                 FROM health_checks
...
//         const query = `
//             SELECT
//                 TRUNC(AVG(hc.response_time_ms)) AS "averageLatencyAll",
//                 TRUNC((COUNT(*) FILTER (WHERE hc.was_successful) * 100.0 / COUNT(*)), 2) AS "uptimePercentageAll",
//                 COUNT(*) FILTER (WHERE NOT hc.was_successful) AS "totalFailuresAll"
//             FROM health_checks hc
//             JOIN monitors m ON hc.monitor_id = m.id
//             WHERE m.user_id = $1 AND hc.timestamp >= NOW() - $2::interval;
//         `;
//         const { rows } = await db.query(query, [req.user.id, intervalMap[period]]);
//         res.status(200).json({ status: 'success', statusCode: 200, message: `User-wide overview for the last ${period} retrieved.`, data: rows[0] });
//     } catch (error) { next(error); }
// });

// // 7. NEW: Worst Performing Monitors
// analyticsRouter.get('/user/worst-performers', async (req, res, next) => {
//     try {
...
//             SELECT
//                 m.id, m.name,
//                 TRUNC(AVG(hc.response_time_ms)) as "averageLatency",
//                 TRUNC((COUNT(*) FILTER (WHERE hc.was_successful) * 100.0 / COUNT(*)), 2) AS "uptimePercentage"
//             FROM health_checks hc
//             JOIN monitors m ON hc.monitor_id = m.id
//             WHERE m.user_id = $1 AND hc.timestamp >= NOW() - $2::interval
//             GROUP BY m.id, m.name
//             ORDER BY "uptimePercentage" ASC, "averageLatency" DESC
//             LIMIT 5;
//         `;
//         const { rows } = await db.query(query, [req.user.id, intervalMap[period]]);
//         res.status(200).json({ status: 'success', statusCode: 200, message: `Worst performing monitors for the last ${period} retrieved.`, data: rows });
//     } catch (error) { next(error); }
// });

// // 8. NEW: Error Breakdown by Message
// analyticsRouter.get('/:monitorId/error-breakdown', async (req, res, next) => {
//     try {
//         const { monitorId } = req.params;
//         const { period = '30d' } = req.query;
//         const intervalMap = { '24h': '1 day', '7d': '7 days', '30d': '30 days' };
...
//         const query = `
//             SELECT
//                 SUBSTRING(error_message for 50) as error, -- Truncate long messages
//                 COUNT(*) as count
//             FROM health_checks
//             WHERE was_successful = false AND monitor_id = $1 AND timestamp >= NOW() - $2::interval AND error_message IS NOT NULL
//             GROUP BY error
//             ORDER BY count DESC
//             LIMIT 10;
//         `;
//         const { rows } = await db.query(query, [monitorId, intervalMap[period]]);
//         res.status(200).json({ status: 'success', statusCode: 200, message: `Error breakdown for the last ${period} retrieved.`, data: rows });
//     } catch (error) { next(error); }
// });
...
const ensureSchema = async () => {
  ...
  try {
    ...
    await db.query(createLocksTableQuery); console.log('✅ "locks" table is ready.');
    console.log('Schema setup complete.');
...
// =================================================================
// 4. POLLING ENGINE WITH DATABASE LOCK
// =================================================================
const runHealthChecks = async () => {
    const client = await db.connect();
    try {
        ...
        const lockResult = await client.query(`
            INSERT INTO locks (lock_name, locked_at) VALUES ('runHealthChecks', NOW())
            ON CONFLICT (lock_name) DO UPDATE SET locked_at = NOW()
            RETURNING (SELECT locked_at FROM locks WHERE lock_name = 'runHealthChecks' AND locked_at > NOW() - INTERVAL '2 minutes');
        `);
        // A more robust check to ensure the lock was actually acquired by this instance
        const lockAcquired = lockResult.rows.length > 0 && lockResult.rows[0].locked_at !== null;
        ...
        if (dueMonitors.length > 0) {
            console.log(`Found ${dueMonitors.length} monitor(s) to check.`);
            for (const monitor of dueMonitors) {
                // Each monitor check is independent, no transaction cascade
                const startTime = Date.now();
                let checkResult = { statusCode: null, responseTime: null, wasSuccessful: false, errorMessage: null };
                try {
                    const response = await axios({ method: monitor.http_method, url: `${monitor.base_url}${monitor.endpoint}`, timeout: 10000 });
                    let isActuallySuccessful = true;
                    const expectedCode = monitor.expected_status_code;
                    if (expectedCode && response.status !== expectedCode) { isActuallySuccessful = false; checkResult.errorMessage = `Expected status ${expectedCode} but received ${response.status}.`; }
                    else if (!expectedCode && (response.status < 200 || response.status >= 300)) { isActuallySuccessful = false; checkResult.errorMessage = `Received non-success status code: ${response.status}.`; }
                    const expectedBody = monitor.expected_response_body;
                    if (isActuallySuccessful && expectedBody) {
                        const bodyString = JSON.stringify(response.data);
                        if (!bodyString.includes(expectedBody)) { isActuallySuccessful = false; checkResult.errorMessage = `Response body did not include expected text.`; }
                    }
                    checkResult.wasSuccessful = isActuallySuccessful;
                    checkResult.statusCode = response.status;
                } catch (error) {
...
// 6. API ROUTES
// =================================================================
app.get('/', (req, res) => { /* ... (This section is unchanged) ... */ const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Klarity API</title><style>body { background-color: #111827; color: #e5e7eb; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; font-family: 'Inter', sans-serif; } h1 { font-size: 4rem; font-weight: 800; letter-spacing: -0.05em; }</style></head><body><h1>Klarity API</h1></body></html>`; res.send(html);});
app.get('/api/v1/health', (req, res) => res.status(200).json({ status: 'ok', statusCode: 200, message: 'API is healthy and running.' }));
app.get('/api/v1/scheduler/run-checks', async (req, res) => { /* ... (This section is unchanged) ... */ if (req.headers.authorization !== `Bearer ${process.env.CRON_SECRET}`) { return res.status(401).json({ status: 'error', message: 'Unauthorized' }); } await runHealthChecks(); res.status(200).json({ status: 'success', statusCode: 200, message: 'Health checks completed successfully.' }); });
const authRouter = express.Router(); /* ... (This section is unchanged) ... */
const signAndSendToken = (user, statusCode, res) => { const token = jwt.sign({ user: { id: user.id } }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN }); const cookieOptions = { expires: new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000), httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax' }; res.cookie('jwt', token, cookieOptions); user.password_hash = undefined; res.status(statusCode).json({ status: 'success', statusCode, message: statusCode === 201 ? 'User registered successfully.' : 'User logged in successfully.', token, data: { user } }); };
authRouter.post('/register', async (req, res, next) => { try { const { email, password } = req.body; if (!email || !password || password.length < 8) { return next(new AppError('Please provide a valid email and a password of at least 8 characters.', 400)); } const userCheck = await db.query('SELECT * FROM users WHERE email = $1', [email]); if (userCheck.rows.length > 0) { return next(new AppError('A user with this email already exists.', 409));} const salt = await bcrypt.genSalt(10); const passwordHash = await bcrypt.hash(password, salt); const newUserQuery = 'INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id, email, created_at'; const { rows } = await db.query(newUserQuery, [email, passwordHash]); signAndSendToken(rows[0], 201, res); } catch (error) { next(error); } });
authRouter.post('/login', async (req, res, next) => { try { const { email, password } = req.body; if (!email || !password) { return next(new AppError('Please provide email and password.', 400));} const userQuery = await db.query('SELECT * FROM users WHERE email = $1', [email]); const user = userQuery.rows[0]; if (!user || !(await bcrypt.compare(password, user.password_hash))) { return next(new AppError('Invalid credentials.', 401)); } const updateLoginQuery = 'UPDATE users SET last_login_at = NOW() WHERE id = $1 RETURNING *'; const { rows } = await db.query(updateLoginQuery, [user.id]); signAndSendToken(rows[0], 200, res); } catch (error) { next(error); } });
authRouter.post('/logout', (req, res) => { res.cookie('jwt', 'loggedout', { expires: new Date(Date.now() + 10 * 1000), httpOnly: true }); res.status(200).json({ status: 'success', statusCode: 200, message: 'User logged out successfully.' }); });
authRouter.get('/me', protect, (req, res) => { res.status(200).json({ status: 'success', statusCode: 200, message: 'User profile retrieved successfully.', data: { user: req.user } }); });
app.use('/api/v1/auth', authRouter);
const monitorRouter = express.Router(); /* ... (This section is unchanged) ... */
monitorRouter.use(protect);
monitorRouter.post('/', async (req, res, next) => { try { const { name, http_method, base_url, endpoint, check_interval_seconds, notification_email, expected_status_code, expected_response_body } = req.body; const query = `INSERT INTO monitors (user_id, name, http_method, base_url, endpoint, check_interval_seconds, notification_email, expected_status_code, expected_response_body) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *;`; const { rows } = await db.query(query, [req.user.id, name, http_method, base_url, endpoint, check_interval_seconds, notification_email, expected_status_code, expected_response_body]); res.status(201).json({ status: 'success', statusCode: 201, message: 'Monitor created successfully.', data: { monitor: rows[0] } }); } catch (error) { next(error); } });
monitorRouter.get('/', async (req, res, next) => { try { const { rows } = await db.query('SELECT * FROM monitors WHERE user_id = $1 ORDER BY created_at DESC;', [req.user.id]); res.status(200).json({ status: 'success', statusCode: 200, message: 'Monitors retrieved successfully.', results: rows.length, data: { monitors: rows } }); } catch (error) { next(error); } });
monitorRouter.get('/:id', async (req, res, next) => { try { const { rows } = await db.query('SELECT * FROM monitors WHERE id = $1 AND user_id = $2;', [req.params.id, req.user.id]); if (rows.length === 0) { return next(new AppError('Monitor not found.', 404)); } res.status(200).json({ status: 'success', statusCode: 200, message: 'Monitor retrieved successfully.', data: { monitor: rows[0] } }); } catch (error) { next(error); } });
...
// 1. Monitor Overview - ACCURATE
analyticsRouter.get('/:monitorId/overview', async (req, res, next) => {
    try {
        const { monitorId } = req.params; const { period = '24h' } = req.query; const intervalMap = { '24h': '1 day', '7d': '7 days', '30d': '30 days' }; if (!intervalMap[period]) { return next(new AppError('Invalid period. Use 24h, 7d, or 30d.', 400)); }
        const statsQuery = `SELECT COUNT(*) AS "totalChecks", TRUNC(AVG(response_time_ms)) AS "averageLatency", TRUNC((COUNT(*) FILTER (WHERE was_successful) * 100.0 / COUNT(*)), 2) AS "uptimePercentage", COUNT(*) FILTER (WHERE NOT was_successful) AS "totalFailures", TRUNC(percentile_cont(0.95) WITHIN GROUP (ORDER BY response_time_ms)) AS "p95Latency", TRUNC(percentile_cont(0.99) WITHIN GROUP (ORDER BY response_time_ms)) AS "p99Latency" FROM health_checks WHERE monitor_id = $1 AND timestamp >= NOW() - $2::interval;`;
        const statusQuery = `SELECT status_code, COUNT(*) as count FROM health_checks WHERE monitor_id = $1 AND timestamp >= NOW() - $2::interval AND status_code IS NOT NULL GROUP BY status_code ORDER BY count DESC;`;
        const [statsResult, statusResult] = await Promise.all([ db.query(statsQuery, [monitorId, intervalMap[period]]), db.query(statusQuery, [monitorId, intervalMap[period]]) ]);
        res.status(200).json({ status: 'success', statusCode: 200, message: `Overview for the last ${period} retrieved.`, data: { ...statsResult.rows[0], statusCounts: statusResult.rows } });
    } catch (error) { next(error); }
});
...
// 3. Incidents Log - ACCURATE
analyticsRouter.get('/:monitorId/incidents', async (req, res, next) => {
    try {
        const { monitorId } = req.params; const { period = '30d' } = req.query; const intervalMap = { '24h': '1 day', '7d': '7 days', '30d': '30 days' }; if (!intervalMap[period]) { return next(new AppError('Invalid period.', 400)); }
        const query = `
            WITH incident_groups AS (
                SELECT timestamp, was_successful, ROW_NUMBER() OVER (ORDER BY timestamp) - ROW_NUMBER() OVER (PARTITION BY was_successful ORDER BY timestamp) as group_id
                FROM health_checks WHERE monitor_id = $1 AND timestamp >= NOW() - $2::interval
            )
            SELECT MIN(timestamp) as start_time, MAX(timestamp) as end_time, EXTRACT(EPOCH FROM (MAX(timestamp) - MIN(timestamp))) as duration_seconds
            FROM incident_groups WHERE was_successful = false GROUP BY group_id ORDER BY start_time DESC;
        `;
        const { rows } = await db.query(query, [monitorId, intervalMap[period]]);
        res.status(200).json({ status: 'success', statusCode: 200, message: `Downtime incidents for the last ${period} retrieved.`, data: rows });
    } catch (error) { next(error); }
